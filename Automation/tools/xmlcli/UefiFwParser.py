	...
	for FvCount in range (0, 8000):
		...
		if( (FvZeroVect == ZeroGuid) and (FvGuid != ZeroGuid) ):		# Every valid FV needs to have this zero vector. 
			...
			for FfsCount in range (0, 8000):
				...
				if ( (FFsGuid == AllFsGuid) and (FFSsize == 0xFFFFFF) ):
	...
	while(BiosKnobBinPtr < BiosKnobBinEndAddr):
		...
		if( (BinHdrSig == "$NVAR") and (KnobCount != 0) ):
			...
			if (NvarSize == 0):
				...
				tmpBiosKnobBinPtr = BiosKnobBinPtr + BIOS_KNOBS_DATA_BIN_HDR_SIZE_OLD
			else:	# New Format
				OldBinFileFormat = False
				...
				tmpBiosKnobBinPtr = BiosKnobBinPtr + BIOS_KNOBS_DATA_BIN_HDR_SIZE
			BiosKnobDict[VarId]={'HiiVarId':0xFF, 'HiiVarSize':0, 'KnobDict':{}, 'DupKnobDict':{}, 'NvarName':NvarName, 'NvarSize':NvarSize, 'KnobCount':KnobCount}
			TmpKnobDict = {}
			...
			BiosKnobBinPtr = BiosKnobBinPtr + BIOS_KNOBS_DATA_BIN_HDR_SIZE
	return BiosKnobDict
...
def ParseNvram(NvRamFvListBuffer, BiosKnobDict, NvRamPointer=0, LogFile=0):
	...
	for VarStrHdrCount in range (0, 0x100):
		...
		PrintLog ("------------|--------|------------|------------|---------------------------", LogFile)
		PrintLog (" CurrentPtr | State  | Atribute   | NvarDataSz |   VarName ", LogFile)
		PrintLog ("------------|--------|------------|------------|---------------------------", LogFile)
		for count in range (0, 200):
			...
			PrintLog (" 0x%-8X |  0x%02X  | 0x%08X | 0x%-8X | L\"%s\" " %(CurVarPtr, VarState, VarAtri, VarDataSize, VarName), LogFile)
			PrintLog ("------------|--------|------------|------------|---------------------------", LogFile)
			if(BiosKnobDictLen):
				for VarId in BiosKnobDict:
					if(VarName == BiosKnobDict[VarId]['NvarName']):
						NvRamDict[VarId] = { 'NvarName':VarName, 'NvarSize':VarDataSize, 'VarAttri':VarAtri, 'NvarDataBufPtr':(CurVarPtr+HdrSize+VarNameSize) }
						break
			else:
				NvRamDict[VarCount] = { 'NvarName':VarName, 'NvarSize':VarDataSize, 'VarAttri':VarAtri, 'NvarDataBufPtr':(CurVarPtr+HdrSize+VarNameSize) }
				VarCount = VarCount + 1
			...
			if(HiiLstGuid == gEfiIfrTianoGuid):
				...
				if (IfrOpcode == EFI_IFR_GUID_OP):
					...
					if(StartFound):
						...
						ReturnAddrDict['IfrList'].append(TmpHiiDbPtr)
...
def ParseIfrForms(HiiDbBinListBuff, BiosKnobDict, HiiStrDict, IfrOpHdrAddr, IfrOpHdrEndAddr, BiosFfsFvBase, FfsFilecount, outXml, LogFile):
	...
	for OpcodeCount in range (0, 0xFFFF):
		...
		if ( (IfrOpcode == EFI_IFR_VARSTORE_OP) or (IfrOpcode == EFI_IFR_VARSTORE_EFI_OP) ):
			...
			for VarId in BiosKnobDict:
				if ( (BiosKnobDict[VarId]['NvarName'] == IfrVarStoreName) and (BiosKnobDict[VarId]['HiiVarId'] == 0xFF) ):
		...
		if(BlockType == EFI_HII_SIBT_END):	# end of string block?
			...
			while(1):
				...
				if(ChrValue):
					...
					break
			HiiStringDict[Prompt] = String.replace('<=', ' &lte; ').replace('>=', ' &gte; ').replace('&', ' &amp; ').replace('\"', '&quot;').replace('\'', '').replace('\x13', '').replace('\x19', '').replace('\xB5', 'u').replace('\xAE', '').replace('<', ' &lt; ').replace('>', ' &gt; ').replace('\r\n', ' ').replace('\n', ' ')
			Prompt = Prompt + 1
	...
	for VarCount in sorted(BiosKnobDict):
		...
		for KnobOffset in sorted(BiosKnobDict[VarCount]['KnobDict']):
			...
			for cnt in range (0, len(PromptList)):
				SetupPgPtr = HiiStrDict.get(PromptList[cnt], "N.A.") + '/' + SetupPgPtr
			if(len(NvramTblDict)):
				try:
...
def FetchBiosId(BiosIdFfsFile):
	...
	if (FfsSize < 0x100):
		for count in range (0, 100):
			ChrVal = ReadList(BiosIdListBuff, (0x24+(count*2)), 1)
			if(ChrVal == 0):
				break
			BiosIdString = BiosIdString + chr(ChrVal)
	else:
...
FileGuidListtoSave  = [ gNvRamFvGuid, gDxePlatformFfsGuid, gBiosKnobsDataBinGuid, gSocketSetupDriverFfsGuid, gSvSetupDriverFfsGuid, gFpgaDriverFfsGuid, gEfiBiosIdGuid, gDefaultDataOptSizeFileGuid, gDefaultDataFileGuid, gClientSetupFfsGuid, gClientTestMenuSetupFfsGuid, gEmulationDriverFfsGuid ]
SetupDriverGuidList = [ gDxePlatformFfsGuid, gSocketSetupDriverFfsGuid, gSvSetupDriverFfsGuid, gFpgaDriverFfsGuid, gClientSetupFfsGuid, gClientTestMenuSetupFfsGuid , gEmulationDriverFfsGuid ]

def GetsetBiosKnobsFromBin(BiosBinaryFile=0, BiosOutSufix=0, Operation="genxml", XmlFilename=0, IniFile=0, UpdateHiiDbDef=False, BiosOut=""):
	...
	for count in range (0, len(SetupDriverGuidList)):
		...
		for IfrFormPkgCount in range (0, (len(HiiPkgAddrDict['IfrList']))):
			IfrOpHdrAddr = ParseIfrForms(HiiDbBinListBuff, BiosKnobDict, HiiStrDict, HiiPkgAddrDict['IfrList'][IfrFormPkgCount], 0, BiosFfsFvBase, count, outXml, LogFile)
