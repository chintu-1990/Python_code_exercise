...
Setting Interface Type:
	Need to select the interface to indicate the scripts on which access method to use (depending on which enviournment we expect the script to operate in).
		...
		cli.clb._setCliAccess("simics")		# For using simics as interface (Pre-Silicon), need to have simics S/W installed on the system
		cli.clb._setCliAccess("svos")		# For using svos as interface, need to have svos disk installed on the system
		cli.clb._setCliAccess("win")		# For using RW.exe as primary Windows interface
		cli.clb._setCliAccess("winhwapi")	# For using HWAPI as primary Windows interface, need to have "Intel® CCG Platform Tools SDK" installed on the system
...
Running popular comands:
	...
	# Step to check XmlCli capability on current System, Please ensure to rename the EnableXmlCli.pyc_ under xmlclitools folder back to EnableXmlCli.pyc
