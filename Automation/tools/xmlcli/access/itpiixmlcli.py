...
_log = _tools.getLogger("itpiixmlcli")
_log.setFile("itpiixmlcli.log",dynamic=True)
_log.setFileFormat("simple")
_log.setFileLevel("info")
_log.setConsoleLevel("result")

class itpiiAccess(_cliutil.cliaccess):
	def __init__(self):
		super(itpiiAccess,self).__init__("itpii")
...
def isThreadAlive(thread):
	return (thread.device.isenabled and getattr(thread,"isvalid", True))
...
def itpii_halt(self):
	...
	for tnum,thread in enumerate(itp.threads):
		if thread.cv.isenabled and thread.cv.isrunning:
