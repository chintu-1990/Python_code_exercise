...
# Author:      skgupta5(saurabh.k.gupta@intel.com)
#-------------------------------------------------------------------------------
__author__ = 'skgupta5'
# Cscripts remove end
import sys,re,os,math,copy
...
def createBinFile(bin_f_name,biosmap,inimap,asitis):
	...
	for knob in asitis:
		if biosmap.has_key(knob):
			...
			if int(knobSize_int) < math.ceil(len(knobVal_hex)/2):
				print "Value [%s] of knob [%s] mentioned size[%s] is larger than mentioned size[%s] in xml" %(knobVal,knob,math.ceil(len(knobVal_hex)/2),knobSize )
				continue
			else:
...
def createCpuSvBinFile(binfile,biosmap,offsetmapMod):
	...
	while (i<len(offsetList_sort)):
		...
		if ( hole_pad >= 0):
			...
			print "knob [%s] offset [%s] is overlapping with knob [%s] offset [%s].. hence existing.." %(offsetmapMod[offsetList_sort[i-1]]['name'],offsetList_sort[i-1],offsetmapMod[offsetList_sort[i]]['name'],offsetList_sort[i])
			return(0x1)
		i+=1
	towrite
	BIN=open(binfile,'wb')
