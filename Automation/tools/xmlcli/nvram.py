...
#			self.__dict__[name] = value
	...
	def existAddress(self, index, offset):
		...
		return (index,offset) in self._valueMap
	...
	def getByAddress(self, index, offset):
		...
		assert len(tmp) != 1,"getByAddress found %i knobs with matched VarStoreIndex (0x%x) and Offset (0x%x)."%(len(tmp),varstoreindex, offset)
		return (tmp[0])
	...
	def _addKnob(self,kDef,kValue):
		if _debug:
			...
			if kDef.setupType not in ['numric','checkbox',"oneof"]:
	...
	for xmlKnob in allKnobs:
		...
		for key,value in xmlKnob.items():
			...
			if key in ["default"]:
				...
				setattr(address,key,_pu.str2long(value))
...
class gbtNVRAM(NVRAM):
	...
	def push(self, binfile=None, updatedOnly=True, CmdSubType=_clb.CLI_KNOB_APPEND):
		...
		_clb.triggerSMI(0xF6)	# trigger S/W SMI for CLI
